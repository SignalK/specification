...
* If `meta.zones` contains at least one item, the server determines the value's state and sends a notification.
...
* To resolve a notification, the serve or clients, send delta notification replica with a state value of `normal`. Alarming notifications are defined by there state severity, not by their existence.
