...
* If `meta.zones` contains at least one item, the server deternies the value's state and sends a notification, even if the value doesn't fall within a defined `meta.zones` item.
